...
public:
    ...
    void PrintTree()
    {
        std::cout<<inorder(mRoot)<<std::endl;
        std::list<std::pair<T, size_t>>& tmp_node_list= PrintTreeImpl(mRoot, 0);
...
private:
    ...
    std::string inorder(std::shared_ptr<TreeNode>& node)
    {
        if(!node){
            return "";
        }
...
private:
    ...
    static std::list<std::pair<T, size_t>>& PrintTreeImpl(std::shared_ptr<TreeNode> node, size_t level)
    {
        static std::list<std::pair<T, size_t>> node_list;
        if(!node){
            return node_list;
        }
        node_list = PrintTreeImpl(node->left, level+1);
        node_list = PrintTreeImpl(node->right, level+1);
